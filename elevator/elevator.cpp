...
#define guestWalkTime 80
#define elevatorTickTime 20
#define guestGenerateTime 300
...
void GuestManager::GenerateGuest()
{
	...
	newGuest.Awake(to_string(name++), 60.0 + rand() % 20, curFloor, targetFloor, 1000 + rand() % 1000);
...
bool MessageManager::NewRequest(string request)
{
	...
	else if (elevator.curSize == maxsize)
	{
